...
export function Artist_Opportunities() {
  ...
  const applySubmitHandler = async (event) => {
    ...
    try {
      ...
      const response = await makeAuthenticatedPOSTRequest(artistOpportunityPoints.APPLY_OPPOR_API + `/${OpportunityapplynowPopup?.id}/apply`, { applyAns }, accessToken);
  ...
  const fetchOpportunity = async () => {
    try {
      const response = await makeAuthenticatedGETRequest(artistOpportunityPoints.FETCH_OPPOR_DATA_API, accessToken);
  ...
  const handleAmountChange = (event) => {
    ...
    setSelectedOption(value === amount ? null : value);
  };
  ...
  let eventToMap = filterData.length > 0 ? filterData : jobData;

  useEffect(() => {
    if (maxAmount !== null && minAmount !== "" && maxAmount !== "" && minAmount !== null && (amount === null || amount === "")) {
      const filteredEvents = jobData.filter((event) => {
        const eventBudget = event.budget;
        return eventBudget >= minAmount && eventBudget <= maxAmount;
      });
  ...
  useEffect(() => {
    if (amount === "All") {
      setFilterData([]);
    } else if (amount?.includes("Below 5000")) {
      const filterEvents = jobData.filter((event) => {
        const eventBudget = event.budget;
        return eventBudget <= 5000;
      });
      if (filterEvents.length > 0) setFilterData(filterEvents);
      else setFilterData([]);
    } else if (amount?.includes("Rs 8,000 - Rs 10,000")) {
      const filterEvents = jobData.filter((event) => {
        const eventBudget = event.budget;
        return eventBudget >= 8000 && eventBudget <= 10000;
      });
    ...
    } else if (amount?.includes("Rs 10,000 - Rs 20,000")) {
      const filterEvents = jobData.filter((event) => {
        const eventBudget = event.budget;
        return eventBudget >= 10000 && eventBudget <= 20000;
      });
    ...
    } else if (amount?.includes("Rs 20,000 - Rs 50,000")) {
      const filterEvents = jobData.filter((event) => {
        const eventBudget = event.budget;
        return eventBudget >= 20000 && eventBudget <= 50000;
      });
    ...
    } else if (amount?.includes("Above 50,000")) {
      const filterEvents = jobData.filter((event) => {
        const eventBudget = event.budget;
        return eventBudget >= 50000;
      });
  ...
  useEffect(() => {
    if (language !== "") {
      const eventToChoose = filterData.length > 0 ? filterData : jobData;
      const filterEvents = eventToChoose.filter((event) => {
        const eventLang = event.languages;
        return eventLang?.includes(language);
      });
  ...
  useEffect(() => {
    if (location !== null && location !== "") {
      ...
      const filtered = eventToChoose.filter((item) => item.location.toLowerCase().includes(location.toLowerCase()));
        ...
        <div className="">
          <form className="OpportunitiesPage_SearchSort">
            <div className="OpportunitiesPage_Search">
              <input placeholder="Search for opportunity"></input>
            </div>
            <div className="OpportunitiesPage_Sort">
              ...
              <button type="button" onClick={() => setShowOpportunityFiltersPopup(true)}>
                Filters
              </button>
            </div>
          </form>
            ...
            {showOpportunityFiltersPopup === true && (
              <div className="OppotunitiesPage_allfilters_formpopup_parent">
                <div className="OppotunitiesPage_allfilters_formpopup">
                  <button onClick={() => setShowOpportunityFiltersPopup(null)}>X</button>
                  <form>
                    ...
                    <div className="OppotunitiesPage_allfilters_form_inputfield">
                      ...
                      <input placeholder="Enter duration week/month"></input>
                    </div>
                    <div className="OppotunitiesPage_allfilters_form_inputfield">
        ...
        <div className="OpportunitiesPage_allfilters">
          <h5>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="26" height="16" viewBox="0 0 26 16" fill="none">
              <path d="M1 1H25M5.28571 8H20.7143M10.4286 15H15.5714" stroke="#AD2F3B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
            </svg>{" "}
            All filters
          </h5>
          <div className="OppotunitiesPage_allfilters_form">
            <form>
              <div className="OppotunitiesPage_allfilters_form_inputfield">
                <label>Location</label>
                <input style={{ fontWeight: "500" }} name="location" onChange={locationHandler} value={location} type="text" />
              </div>
              <div className="OppotunitiesPage_allfilters_form_inputfield">
                <label>Amount</label>
                <div className="minmaxamount">
                  <label>Min</label>
                  <input onChange={handleMinAmountChange} value={minAmount} type="number" />

                  <label>Max</label>
                  <input onChange={handleMaxAmountChange} value={maxAmount} type="number" />
                </div>
                <div className="minmaxamountradio">
                  {filterAmount.map((data, index) => (
                    <label key={index}>
                      <input type="radio" name="amount" onClick={handleOptionChange} value={data.title} checked={amount === data.title} onChange={handleAmountChange} />
                      {data.title}
                    </label>
                  ))}
                </div>
              ...
              <div className="OppotunitiesPage_allfilters_form_inputfield">
                <label>Language</label>
                <select value={language} name="language" onChange={languageHandler}>
            ...
            <div className="OpportunitiesPage_displayonejob" key={index}>
              ...
              <div className="OpportunitiesPage_displayonejob_content">
                ...
                <div className="OpportunitiesPage_displayonejob_contentdetailsbuttons">
                  ...
                  <button className="btntwo" onClick={() => setOpportunityapplynowPopup({ index, id: job._id })}>
                    Apply Now
                  </button>
                </div>
                ...
                {OpportunityapplynowPopup?.index === index && (
                  <div className="Opportunityapplynowpopup_fullscreen">
                    <div className="Opportunityapplynowpopup">
                      <button onClick={() => setOpportunityapplynowPopup(null)}>X</button>
                      ...
                      <div className="Opportunityapplynowpopup_contentform">
                        <form onSubmit={applySubmitHandler}>
                          ...
                          <button onClick={() => setOpportunityapplynowPopup(null)}>Cancel</button>
                        </form>
                      </div>
                    </div>
                  </div>
