    ...
    try {
      ...
      const response = await makeAuthenticatedPOSTRequest(
        artistOpportunityPoints.APPLY_OPPOR_API +
          `/${OpportunityapplynowPopup?.id}/apply`,
        { applyAns },
        accessToken
      );
...
useEffect(()=>{
  if(location !== null && location !== ""){
    ...
    const filtered = eventToChoose.filter(item =>
            ...
            <form>
              ...
              <div className="OppotunitiesPage_allfilters_form_inputfield">
                ...
                <select value={language} name="language" onChange={languageHandler}  >
                    <option value="" defaultChecked selected  >Choose Language</option>
                    {
                        AllLanguage.map((lan ,index)=>(
                            <option value={lan}  key={index}>{lan}</option>
                        ))
                    }
                </select>
              </div>
